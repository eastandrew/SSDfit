...
m <- matrix(data=NA, nrow=nsamps, ncol=nreps)
for (i in 2:nsamps) {
  for (j in 1:nreps) {
...
m2$predict <- c("NA",predict(segmean))
m2$predictmed <- c("NA",predict(segmedian))
...
plot(medianv~sampsize, data=m2, pch=16, log="", ylim=c(0,3))
points(highCI~sampsize, data=m2, type="l", lty=2)
points(lowCI~sampsize, data=m2, type="l", lty=2)
points(predictmed~sampsize, data=m2, type="l", lty=2, col="red")
