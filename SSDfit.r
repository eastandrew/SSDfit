...
ssddata$percorder <- 1.05-order(ssddata$NOEC, decreasing=T)/max(ssddata$idnum)
ssddata$percorder2 <- rank(ssddata$NOEC)/max(ssddata$idnum)
ssddata


plot(percorder2~NOEC, data=ssddata, log="x")
plot(percorder~NOEC, data=ssddata, log="x")
...
fitobj$sd[1]
fitobj$sd[2]
meandist <- abs(rnorm(100,mean=fitobj$estimate[[1]],sd=fitobj$sd[[1]]))
sddist <- abs(rnorm(100,mean=fitobj$estimate[[2]],sd=fitobj$sd[[2]]))
...
points(ssddata$percorder~ssddata$NOEC,pch=16,col="red")
points(ssddata$percorder~ssddata$NOEC,type="S",col="red")
...
dfbig95$highCI[dfbig95$prob==0.05]
qlnorm(0.05,meanlog=fitobj$estimate[[1]],sdlog=fitobj$estimate[[2]],lower.tail=T)
median(dfbigtest$estimate[dfbigtest$prob==0.05])

plot(density(dfbigtest$estimate[dfbigtest$prob==0.05]))
plot(density(dfbigtest$estimate[dfbigtest$prob==0.05]), xlim=c(0,1))
segments(c(dfbig95$lowCI[dfbig95$prob==0.05], median(dfbigtest$estimate[dfbigtest$prob==0.05]),qlnorm(0.05,meanlog=fitobj$estimate[[1]],sdlog=fitobj$estimate[[2]],lower.tail=T)
, dfbig95$highCI[dfbig95$prob==0.05]),c(0,0,0,0),c(dfbig95$lowCI[dfbig95$prob==0.05], median(dfbigtest$estimate[dfbigtest$prob==0.05]), qlnorm(0.05,meanlog=fitobj$estimate[[1]],sdlog=fitobj$estimate[[2]],lower.tail=T)
,dfbig95$highCI[dfbig95$prob==0.05]),c(0.4,0.4,0.4,0.4), lty=2,col=c("black","blue","red","black"))
